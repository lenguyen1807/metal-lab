...
DispatchPlan naive_dispatch(const GemmShape& shape,
                            const MTL::ComputePipelineState&)
{
  constexpr size_t threads_x = 32;
  constexpr size_t threads_y = 32;
  return {MTL::Size::Make((shape.N + threads_x - 1) / threads_x,
                          (shape.M + threads_y - 1) / threads_y, 1),
          MTL::Size::Make(threads_x, threads_y, 1)};
}

bool all_shapes(const GemmShape&) { return true; }

void encode_naive(MTL::ComputeCommandEncoder& encoder,
                  ...
                  const DispatchPlan& plan)
{
  ...
  encoder.dispatchThreadgroups(plan.threadgroups,
...
const std::vector<KernelSpec>& kernel_specs()
{
  static const std::vector<KernelSpec> specs = {
      {"naive", "matmul_naive", naive_dispatch, all_shapes, encode_naive}};
  return specs;
}

Kernel::Kernel(KernelSpec spec, MTL::Device* device) : spec_(spec)
{
  if (device == nullptr || spec_.dispatch == nullptr
      || spec_.encode == nullptr) {
    throw std::invalid_argument("Kernel requires a device and launch callbacks");
  }
  auto pool = NS::TransferPtr(NS::AutoreleasePool::alloc()->init());
  NS::Error* error = nullptr;
  const std::string source = read_file(
      std::string(KERNEL_PATH) + spec_.name + ".metal");
  auto* metal_source = NS::String::string(
      source.c_str(), NS::StringEncoding::UTF8StringEncoding);
  library_ = NS::TransferPtr(device->newLibrary(metal_source, nullptr, &error));
  if (!library_) {
    const char* message = error
        ? error->localizedDescription()->utf8String()
        : "Metal returned no library";
    throw std::runtime_error("Cannot compile " + std::string(spec_.name)
                             + ": " + message);
  }

  auto* function_name = NS::String::string(
      spec_.function, NS::StringEncoding::UTF8StringEncoding);
  function_ = NS::TransferPtr(library_->newFunction(function_name));
  if (!function_) {
    throw std::runtime_error("Cannot find shader function "
                             + std::string(spec_.function));
  }
  error = nullptr;
  pipeline_ = NS::TransferPtr(
      device->newComputePipelineState(function_.get(), &error));
  if (!pipeline_) {
    const char* message = error
        ? error->localizedDescription()->utf8String()
        : "Metal returned no pipeline";
    throw std::runtime_error("Cannot create pipeline for "
                             + std::string(spec_.name) + ": " + message);
  }
}
...
void Kernel::encode(MTL::CommandBuffer* command_buffer,
                    const DeviceMatrix& A,
                    const DeviceMatrix& B,
                    DeviceMatrix& C) const
{
  const GemmShape shape{static_cast<uint32_t>(C.rows),
                        static_cast<uint32_t>(C.cols),
                        static_cast<uint32_t>(A.cols)};
  if (!supports(shape) || A.cols != B.rows || A.rows != C.rows
      || B.cols != C.cols) {
    throw std::invalid_argument("Unsupported GEMM shape for "
                                + std::string(spec_.name));
  }
  ...
  if (threads.width == 0 || threads.height == 0 || threads.depth == 0
    ...
    throw std::runtime_error("Invalid threadgroup size for "
                             + std::string(spec_.name));
  }

  auto encoder = NS::RetainPtr(command_buffer->computeCommandEncoder());
  if (!encoder) {
    throw std::runtime_error("Cannot create compute encoder for "
                             + std::string(spec_.name));
  }
